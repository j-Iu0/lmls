...
from lmls.core.config import ConfigError, config_from_dict, config_to_dict, load_config
...
def test_editor_roundtrip_preserves_unknown_settings_and_disabled_nodes(format):
    if format == 'toml':
...
def test_editor_rejects_undeclared_subscription_ports():
    doc = to_editor(load_config(REPO / 'config/mock.toml'))
    screen = next(n for n in doc['nodes'] if n['name'] == 'screen')
    screen['inputs'] = {'implicit_port': 'text.out'}

    with pytest.raises(ConfigError, match='not declared by PrettyStdoutSink'):
        from_editor(doc)
