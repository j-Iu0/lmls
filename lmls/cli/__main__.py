"""``lmls`` -- run the whole pipeline, inspect a graph, or benchmark a preset.

    lmls run --config config/default.toml
    lmls run --config config/mock.toml            # no models needed
    lmls run --chain mic,segment,asr,translate --target vi
    lmls graph --config config/default.toml --mermaid
    lmls bench --config config/mock.toml --in assets/lecture.wav
    lmls devices
"""
