"""``python -m lmls.translate`` -- translation, standalone.
...
without a repair mode are swapped for their fused counterpart, which does the same job:
    ...
    lmls.transcribe run mock --in x.wav | lmls.correct run mlx_llm_corrector | lmls.translate run mlx_llm_translator
    lmls.transcribe run mock --in x.wav | lmls.translate run mlx_llm_translator --repair
    lmls.transcribe run mock --in x.wav | lmls.translate run ollama_translator --repair   # -> fused_ollama
"""
