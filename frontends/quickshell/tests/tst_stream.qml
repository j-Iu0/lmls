...
TestCase {
    ...
    function test_receive() {
        ...
        peer.sendTextMessage(JSON.stringify({type: "subtitle", port: "raw",
        ...
        peer.sendTextMessage(JSON.stringify({type: "subtitle", port: "translated",
            segment_id: "u1", revision: 0, lang: "zh", text: "直播字幕",
            t_audio_end: Date.now() / 1000, t_emit: Date.now() / 1000}));
        tryCompare(stream, "translationText", "直播字幕");
        compare(stream.translationLang, "zh");
        compare(stream.history[0].translationLang, "zh");
